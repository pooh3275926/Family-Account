...
const MemoManagerView: React.FC = () => {
    ...
    const { journalEntries, accounts } = data;
    ...
    const filteredMemos = useMemo(() => {
        ...
        return memosByAccount
            .map(([accountId, accountData]) => {
                const filteredMemoEntries = Array.from(accountData.memos.entries()).filter(([memo]) =>
                    memo.toLowerCase().includes(lowerSearch)
                );
                if (accountData.accountName.toLowerCase().includes(lowerSearch) || filteredMemoEntries.length > 0) {
                     return [accountId, { ...accountData, memos: new Map<string, number>(filteredMemoEntries) }];
                }
                return null;
            })
    ...
    return (
        <div className="bg-stone-900 p-4 sm:p-6 rounded-xl shadow-lg">
            ...
            <div className="space-y-2 max-h-[70vh] overflow-y-auto pr-2">
                {filteredMemos.map(([accountId, accountData]) => (
                    <div key={accountId} className="border border-stone-700 rounded-lg">
                        <button onClick={() => toggleAccount(accountId)} className="w-full flex justify-between items-center p-3 bg-stone-800 hover:bg-stone-700/80 transition-colors">
                            <span className="font-semibold text-stone-200 text-left">{accountData.accountName}</span>
                            {expandedAccounts.has(accountId) ? <ChevronDown /> : <ChevronRight />}
                        </button>
                        {expandedAccounts.has(accountId) && (
                            <ul className="p-2">
                                {Array.from(accountData.memos.entries()).map(([memo, count]) => (
