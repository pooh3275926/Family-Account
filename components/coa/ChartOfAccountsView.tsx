...
const ChartOfAccountsView: React.FC = () => {
    ...
    const { accounts, journalEntries, creditCardLedgers, amortizationItems } = data;
    ...
    const confirmDelete = () => {
        if (accountToDelete) {
            const isAccountInUse = journalEntries.some(entry => 
                entry.lines.some(line => line.accountId === accountToDelete)
            );
            
            const isLiabilityAccountInUse = creditCardLedgers.some(ledger => ledger.liabilityAccountId === accountToDelete);
            
            const isCreditAccountInUse = amortizationItems.some(item => item.creditAccountId === accountToDelete);
            const isDebitAccountInUse = amortizationItems.some(item => item.debitAccountId === accountToDelete);
