...
import { BookText, FileText, BarChart2, Settings, CreditCard, Menu, X, ClipboardList, ArchiveRestore, BookCopy, Briefcase, LayoutDashboard, Zap, ChevronDown, Database, Users } from 'lucide-react';
...
import MemoManagerView from './components/prepayments/MemoManagerView';
...
import SyncAndBackupView from './components/system/SyncAndBackupView';
import UserSelectionView from './components/system/UserSelectionView';
import LoadingView from './components/system/LoadingView';


type Page = 'dashboard' | 'journal' | 'credit-card' | 'salary' | 'closing' | 'coa' | 'amortization' | 'prepayments' | 'received-payments' | 'memos' | 'balance-sheet' | 'income-statement' | 'sync-and-backup';
...
const MainApp: React.FC = () => {
    ...
    const navItems = useMemo(() => (
        <div className="space-y-4">
            ...
            <NavSection title="功能選單" isOpen={isFeaturesOpen} onToggle={() => setIsFeaturesOpen(!isFeaturesOpen)}>
                ...
                <NavItem page="sync-and-backup" label="資料同步與備份" icon={Database} />
            </NavSection>
    ...
    if (state.isLoading) {
        return <LoadingView message="正在載入資料..." />;
    }

    if (state.googleUser && !state.activeProfileId) {
    ...
    const renderPage = () => {
        switch (currentPage) {
            ...
            case 'sync-and-backup': return <SyncAndBackupView />;
                ...
                <nav className={`flex-1 overflow-y-auto p-4 transition-opacity duration-300 ${isDesktopSidebarCollapsed ? 'opacity-0 md:opacity-100' : 'opacity-100'}`}>
                   {isDesktopSidebarCollapsed ? (
                        <div className="flex flex-col items-center space-y-3">
                            ...
                            <span title="資料同步與備份"><Database onClick={() => handleSetPage('sync-and-backup')} className="cursor-pointer text-stone-500 hover:text-stone-200" /></span>
                            <div className="w-full border-t border-stone-600 my-2"></div>
